...
- Keep the VPS-focused Mullvad configs under `/etc/wireguard` but inject the SSH-preserving `PostUp`/`PreDown` rules directly there. Docker gets its own dedicated configs by running `mullvad-wg.sh` *inside* the custom VPN container (see `docker/images/wg-vpn`; composed via `docker/compose/docker-compose.mullvad-wg.yml`). The compose file mounts `/opt/abb-docker/state/wg-profiles` to persist container-only profiles, and the container rotates to a random profile every 15 minutes (adjustable via `WG_ROTATE_SECONDS`). Trigger manual swaps with `/opt/abb-docker/scripts/rotate-wg.sh` when necessary.
...
- ProtonVPN:
  ...
  - For Docker-based workflows, ship both a Gluetun stack (WireGuard configs supplied via env file) and an OpenVPN gateway stack that consumes operator-supplied ProtonVPN `.ovpn` profiles from `/opt/openvpn-configs` and rotates them without rebuilding the container. The docker-tools sync step should detect `~/openvpn-configs` (per-user staging area), copy any `.ovpn`/`credentials.txt` files into `/opt/openvpn-configs` with 0700 perms, overwrite colliding names, remove the originals from the home directory, and let the container auto-load `credentials.txt` (or `OPENVPN_AUTH_FILE` / `OPENVPN_AUTH_USER` + `OPENVPN_AUTH_PASS`) for non-interactive authentication.
