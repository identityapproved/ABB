...
- ProtonVPN OpenVPN gateway: copy Proton-generated `.ovpn` profiles (and optional `credentials.txt`) into `~/openvpn-configs`, then rerun `./abb-setup.sh docker-tools` so ABB can secure them under `/opt/openvpn-configs` (root:root 0700) and purge the originals. Copy `/opt/abb-docker/env/openvpn.env.example` to `.env`, tune defaults (set `OPENVPN_AUTH_FILE` if you store creds outside `credentials.txt`), then run `docker compose -f /opt/abb-docker/compose/docker-compose.openvpn.yml up -d --build`. Dependent stacks should keep `network_mode: "service:vpn-gateway"`. Rotate to the next (or random) config with `/opt/abb-docker/scripts/rotate-openvpn.sh` or schedule cron: `(crontab -l 2>/dev/null; echo "*/7 * * * * /opt/abb-docker/scripts/rotate-openvpn.sh >/dev/null 2>&1") | crontab -`. The helper swaps profiles inside the container and sends `SIGHUP`, so the namespace survives the rotation.
