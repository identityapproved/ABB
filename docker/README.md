...
This stack expects you to download ProtonVPN OpenVPN `.ovpn` profiles manually and drop them under `~/openvpn-configs`. Re-run `./abb-setup.sh docker-tools` after adding new configs and the task will move them into `/opt/openvpn-configs` (owned by `root:root`, mode `0700`), overwriting older files with the same name and removing the originals from your home directory. Keep credentials inside the configs or add a `credentials.txt` alongside them; it will be copied securely as well. If `/opt/openvpn-configs/credentials.txt` exists (or you set `OPENVPN_AUTH_FILE` in the env file), the gateway feeds it to `openvpn --auth-user-pass` automatically so no TTY prompts block the container. The resulting `/opt/openvpn-configs` directory is mounted read-only into the container to avoid accidental edits.
