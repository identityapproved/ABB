...
- After reconnecting as the managed user, run `./abb-setup.sh package-manager` to write `/etc/pacman.d/blackarch.conf`, import and locally sign the BlackArch key, enable multilib (if missing), force `pacman -Syyu`, and install/cache your preferred AUR helper (`yay`, `paru`, `pacaur`, `pikaur`, `aura`, or `aurman`).
...
- **BlackArch repo:** The package-manager stage writes `/etc/pacman.d/blackarch.conf`, imports and locally signs the BlackArch key, enables multilib, forces `pacman -Syyu`, and then builds your chosen AUR helper.
