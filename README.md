...
- **WireGuard ready:** Utilities install `wireguard-tools`/`openresolv`; the dedicated `mullvad` task runs `mullvad-wg.sh` once (removing the script afterwards), copies clean configs to `/opt/wg-configs/pool` for Docker, and only injects SSH-preserving rules into the originals under `/etc/wireguard` for VPS usage.
...
1. Start the WireGuard transport:
   ```bash
   cd /opt/abb-docker/compose
   docker compose -f docker-compose.vpn.yml up -d
   ```
2. Run another stack through the VPN container, for example ReconFTW:
   ```bash
   docker compose -f docker-compose.reconftw.yml run --rm reconftw -d example.com -r
   ```
3. Rotate exit IPs whenever needed:
   ```bash
...
- Configs generated by `./abb-setup.sh mullvad` are copied to `/opt/wg-configs/pool` for Docker usage, while the originals in `/etc/wireguard` receive the SSH-preserving PostUp/PreDown rules for VPS sessions. The active config used by Docker lives at `/opt/wg-configs/active/wg0.conf`.
...
- Use `/opt/abb-docker/scripts/rotate-wg.sh` to swap the WireGuard profile used by the VPN container and verify your exit IP automatically.
- Optional auto rotation: add a cron job to call the rotation helper every 15 minutes if you want continuous Mullvad IP changes:
  ```bash
